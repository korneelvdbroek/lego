...
class Buggy:
    def __init__(self):
        ...
        try:
            self.engine_drive1 = Motor(Port.A, Direction.COUNTERCLOCKWISE)
            self.engine_drive2 = Motor(Port.B, Direction.CLOCKWISE)
            self.engine_steer = Motor(Port.C)
        ...
        self.engine_drive1.control.limits(acceleration=1000)
        self.engine_drive2.control.limits(acceleration=1000)
    ...
    def run_online(self):
        ...
        while True:
            ...
            moving = (umath.fabs(self.engine_drive1.speed()) > 2) or (umath.fabs(self.engine_drive2.speed()) > 2)  # int
            if moving:
            ...
            if drive_speed != 0:
                ...
                if angle_deviation < -25:
                    self.engine_drive1.run(drive_speed)
                    self.engine_drive2.stop()
                elif angle_deviation > 25:
                    self.engine_drive1.stop()
                    self.engine_drive2.run(drive_speed)
                else:
                    self.engine_drive1.run(drive_speed)
                    self.engine_drive2.run(drive_speed)
            else:
                direction_target = direction_angle_z
                self.engine_drive1.brake()  # could also use .brake() here to stop more suddenly
                self.engine_drive2.brake()
    ...
    def forward(self, distance_cm=30):
        ...
        self.engine_drive1.run_angle(self.drive_speed, drive_angle, then=Stop.BRAKE, wait=False)
        ...
        watch.reset()
        t0 = 0
        while not self.engine_drive1.done():
            t = watch.time()
    ...
    def turn(self, steer_angle_engine):
        ...
        self.engine_drive1.run(self.drive_speed)
        sleep = 10
        angle_z = 0
        watch = StopWatch()
        watch.reset()
